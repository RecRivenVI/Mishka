...
class SuperIslandBypass(
     ...
     * 因而这里不再重复查询能力。UID deny 必须由独立进程在主进程死亡时恢复；目前只有 Root
     * app_process 提供这项保证，Shizuku hook 与主进程同生命周期，不能安全进入绕过队列。
     */
    fun appliesTo(
        ...
        style == VpnNotificationStyle.MiIsland &&
                ...
                settings.authorizer == Authorizer.Root
        ...
        return runCatching {
            ...
            val window = windowOperationLock.withLock {
                ...
                if (!hadActiveWindow) {
                    ...
                    if (activeSession.crashRecovery?.arm(targetUid, chainWasEnabled) != true) {
                        clearRecoveryMarker()
                        return@withLock null
                    }
                }
                ...
                if (!result.blocked) {
                    if (!hadActiveWindow) activeSession.crashRecovery?.clear()
    ...
    private fun recoverPersistedWindowIfNeeded(session: DirectPrivilegedSession? = null): Boolean {
        ...
        val restored = restoreNetworking(recoverySession, recovery.chainWasEnabled)
    ...
    private fun forceRestoreXmsfNetwork(maxOwnerGeneration: Long) {
        ...
        if (active != null) {
            recoveryNeeded = !restoreNetworking(active.value, active.chainWasEnabled)
            active.value.close()
        }
    ...
    private fun restoreNetworking(
        session: DirectPrivilegedSession,
        chainWasEnabled: Boolean,
    ): Boolean {
        val targetUid = xmsfUid ?: return false
        val restored = XmsfFirewallTransaction.restore(session.operations, targetUid, chainWasEnabled)
        val recoveryCleared = restored && session.crashRecovery?.clear() != false
        if (recoveryCleared) {
            Log.d(TAG, "Restored XMSF network after Super Island bypass")
        } else if (restored) {
            Log.e(TAG, "Restored XMSF network but failed to disarm crash recovery")
        } else {
    ...
    private fun restoreXmsfWindow(window: XmsfWindowLease<DirectPrivilegedSession>) {
        windowOperationLock.withLock {
            ...
            recoveryNeeded = !restoreNetworking(window.value, window.chainWasEnabled)
            window.value.close()
        }
    }
