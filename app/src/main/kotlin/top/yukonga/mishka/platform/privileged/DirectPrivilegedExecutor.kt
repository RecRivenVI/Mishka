...
class DirectPrivilegedSession internal constructor(
    ...
    val crashRecovery: PrivilegedCrashRecovery?,
    private val handle: Closeable,
) : Closeable {
    override fun close() = handle.close()
}

interface PrivilegedCrashRecovery {
    fun arm(uid: Int, chainWasEnabled: Boolean): Boolean
    fun clear(): Boolean
}
    ...
    return when (authorizer) {
        Authorizer.Root -> {
            ...
            DirectPrivilegedSession(
                ...
                crashRecovery = object : PrivilegedCrashRecovery {
                    override fun arm(uid: Int, chainWasEnabled: Boolean): Boolean =
                        handle.entity.armNetworkRecovery(uid, chainWasEnabled)

                    override fun clear(): Boolean = handle.entity.clearNetworkRecovery()
                },
                handle = handle,
            )
        }
